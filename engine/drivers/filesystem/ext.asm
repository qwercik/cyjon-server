...
.loop:
	...
	; czy istnieje blok InDirect?
	cmp	dword [rdi + rax],	VARIABLE_EMPTY
	je	.end

	push	rax

	mov	eax,	dword [rdi + rax]
	call	filesystem_ext_file_read_indirect

	pop	rax

	; czy istnieje blok Double-InDirect?
	add	rax,	VARIABLE_DWORD_SIZE
	cmp	dword [rdi + rax],	VARIABLE_EMPTY
	je	.end
	...
	mov	eax,	dword [rdi + rax]
	call	filesystem_ext_file_read_indirect_double

	pop	rax

	; czy istnieje blok Triple-InDirect?
	add	rax,	VARIABLE_DWORD_SIZE
	cmp	dword [rdi + rax],	VARIABLE_EMPTY
	je	.end

	mov	eax,	dword [rdi + rax]
	call	filesystem_ext_file_read_indirect_triple
...
text_filesystem_ext_flag_extent	db	" Ext driver: no support for Extent flag, driver offline.", VARIABLE_ASCII_CODE_ENTER, VARIABLE_ASCII_CODE_NEWLINE, VARIABLE_ASCII_CODE_TERMINATOR

filesystem_ext_file_read_indirect_triple:
	; zachowaj oryginalne rejestry
	push	rax
	push	rcx
	push	rdi

	; przydziel miejsce na block Triple-InDirect i zachowaj adres
	call	cyjon_page_allocate
	push	rdi

	; wczytaj 1 blok danych
	mov	rcx,	1
	call	filesystem_ext_block_read

	; ustaw ilość rekordów w bloku
	mov	rcx,	qword [variable_filesystem_ext_s_log_block_size_in_bytes]
	shr	rcx,	VARIABLE_DWORD_SIZE / 2

.loop:
	; pobierz następny numer bloku
	mov	eax,	dword [rdi]
	; sprawdź czy istnieją następne bloki
	jz	.end

	call	filesystem_ext_file_read_indirect_double

	add	rdi,	VARIABLE_DWORD_SIZE
	dec	rcx
	jnz	.loop

.end:
	; zwolnij pamięć tymczasową na block Triple-InDirect
	pop	rdi
	call	cyjon_page_release
...
filesystem_ext_file_read_indirect_double:
	; zachowaj oryginalne rejestry
	push	rax
	push	rcx
	push	rdi

	; przydziel miejsce na block Double-InDirect i zachowaj adres
	call	cyjon_page_allocate
	push	rdi
...
.end:
	; zwolnij pamięć tymczasową na block Double-InDirect
	pop	rdi
	call	cyjon_page_release
...
filesystem_ext_file_read_indirect:
	; zachowaj oryginalne rejestry
	push	rax
	push	rcx
	push	rdi

	; przydziel miejsce na block InDirect i zachowaj adres
	call	cyjon_page_allocate
	push	rdi
